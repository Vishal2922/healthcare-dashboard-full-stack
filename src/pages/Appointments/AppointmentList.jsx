...
export default function AppointmentList() {
  ...
  const handleFilterChange  = useCallback((e) => filterByStatus(e.target.value), [filterByStatus]);
  const handleStatusChange  = useCallback((id, s) => updateStatus(id, s), [updateStatus]);
  const handleCancel        = useCallback((id) => { if (window.confirm('Cancel this appointment?')) cancelAppointment(id); }, [cancelAppointment]);
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span style={{ padding: '3px 10px', borderRadius: 12, fontSize: 11, fontWeight: 700, color: s.color, background: s.bg, textTransform: 'uppercase', letterSpacing: 0.4 }}>
        {status}
      </span>
    );
  };
  ...
  const renderPagination = () => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 20, flexWrap: 'wrap' }}>
        ...
        {pages.map((p) => (
          <button key={p} onClick={() => goToPage(p)} disabled={loading} style={{ ...S.pageBtn, ...(p === page ? S.activePage : {}) }}>{p}</button>
        ))}
    ...
    <div style={S.page}>
      ...
      {offlineQueue.length > 0 && (
        <div style={S.queuePanel}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 12, color: '#718096' }}>Will submit automatically on reconnect.</p>
              ...
              {list.map((appt) => {
                ...
                return (
                  <tr key={appt.id} style={{ borderBottom: '1px solid #edf2f7' }}>
                    <td style={S.td}><strong>{appt.patient_name || `#${appt.patient_id}`}</strong></td>
                    <td style={S.td}>{appt.doctor_name || `#${appt.doctor_id}`}</td>
                    <td style={S.td}>
                      <div>{new Date(appt.appointment_time).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</div>
                      <div style={{ fontSize: 12, color: '#718096' }}>{new Date(appt.appointment_time).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}</div>
                    </td>
